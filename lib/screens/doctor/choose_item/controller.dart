...
class ChooseItemController extends StateNotifier<ChooseItemData> {
  ...
  Future<String> saveQuestions() async {
    ChooseItemData st = state;
    dynamic res = await api.getTest(uri: st.uri);
    ...
    res['testParts'][0]['assessmentSections'][0]['sectionParts'] = sectionParts;
    res = await api.saveQuestions(model: json.encode(res), dataUri: st.uri);
    if (res is AppError) return res.description;
    return '';
  }
